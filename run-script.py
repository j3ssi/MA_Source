...
parser.add_argument('-c', '--checkPointNumber', default=1, type=int, help='number of GPUs used in training')
...
def main():
    ...
    checkpoint = './output/prune' + str(args.checkPointNumber)
    s = 3
    for i in range(0,5):
        for j in range(0, 36):
            cmdLine = 'python3 main.py '
            cmdLine = cmdLine + '--workers 0 --checkpoint ' + checkpoint + ' --test --epochs 5 --cifar10 --gpu_id 2 '
