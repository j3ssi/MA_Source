        ...
        for layers in range(0, len(self.module_list)):
            if layers < (2 * k -2):
                ...
                if isinstance(self.module_list[layers], nn.Conv2d):
                    ...
                    module_list.append(self.module_list[layers + 2 * numDelete])
                    print(
                        f'Ersetze {layers} gegen {layers + 2 * numDelete}: {self.module_list[layers]} gegen {self.module_list[layers + 2 * numDelete]}')          elif (layers + 1) % (len(self.module_list)-2 * numDelete) == 0:
                module_list.append(self.module_list[layers + 2 * numDelete])
